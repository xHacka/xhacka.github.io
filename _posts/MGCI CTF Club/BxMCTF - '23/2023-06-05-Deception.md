---
title: BxMCTF '23 - Deception
date: Mon Jun  5 12:39:39 AM +04 2023
categories: [Writeup]
tags: [ctf,bxmctf2023,reversing]
---
...
[rev1.zip](https://ctfmgci.jonathanw.dev/dl/bxmctf2023/rev1.zip)

## Analysis

zip contains a text file called `Answers.txt`, each line seems to be Base64 encoded data, using CyberChef we can easily decode the values.

Decoding 4th line of Base64 gives us some Java code.
```java
import java.io.*;
import java.util.*;
import java.time.*;
...
The Java code seems to take number as input (from 0 to len(table)) and do an encryption of some kind.

But first we have a hint. 

***Open Google Drive Link -> Copy Key -> [Decode Brainf*ck](https://copy.sh/brainfuck/) -> Decode Base64 ->***
> Roses are red,
> Violets are blue,
> If one wants to pick the correct flag,
> Then they should seek the Unix Epoch as a clue  

Hmm... `wants to pick the correct flag`, we probably can't get flag directly from the hint. If we generate all possible keys that Program can generate and sift it we'll probably be able to `pick the correct flag`.
...
└─$ ls
 Deception$FastReader.class   Deception.class   Deception.java

# Generate flags from 1 to len(table)
└─$ for i in {1..1106}; do (echo -n $i | java Deception | grep "ctf") >> guessing.log; done;

# Get the flag              # Epoch start (year)    
└─$ cat guessing.log | grep 1970   
ctf{REDACTED}
```