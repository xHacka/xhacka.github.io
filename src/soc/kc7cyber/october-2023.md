...
### 1. In cyber threat intelligence, what does RFI stand for?
...
### 1. On September 22, 2023, the Google Threat Analysis Group (TAG), in partnership with The Citizen Lab, identified 0-day exploits targeting what type of device?
...
### 1. What automated malware analysis platform includes the name of a bird?
...
### 1. On September 25, 2023, CyberWire reported additional data breaches of victims impacted by a supply chain compromise of which software?
...
### 1. On September 23, 2023, which cryptocurrency was hacked, resulting in it being one of the largest cryptocurrency hacks thefts year?
...
### 1. On October 4, 2023, Atlassian disclosed what CVE vulnerability targeting Confluence servers?
...
### 1. On October 5, 2023, it was reported that which malware has resurged, responsible for now pushing ransomware and backdoors?
...
### 1. In a Ransomware-as-a-Service (RaaS) model, what is the name of the role where the criminal is responsible for launching and deploying the ransomware payload on a compromised victim?
...
### 1. Within the OSI model, which layer is the Transmission Control Protocol (TCP) located in?
...
### 1. What is the acronym commonly used with the attack method where the attacker uses binaries and scripts already present in a compromised environment for malicious purposes?
...
### 1. On October 10, 2023, Microsoft Threat Intelligence tweeted which threat actor was observed exploiting CVE-2023-22515 in the wild?

[https://x.com/MsftSecIntel/status/1711871732644970856](https://x.com/MsftSecIntel/status/1711871732644970856)
...
### 1. On Microsoft BlueHat's Twitter profile, what is the motto displayed on its background photo?
...
### 1. At Microsoft BlueHat, who from the KC7 team presented on the KC7 platform?
...
### 1. On October 11, 2023, Microsoft plans to eliminate which legacy authentication method in Windows 11?
...
### 1. Which person from the KC7 team is speaking at Cyberwarcon?

- Google: `Cyberwarcon 2023 speakers "KC7cyber"`
- Find twitter: [https://x.com/simandsec](https://x.com/simandsec)
...
### 1. Which North Korean group was responsible for the 2009 Operation Troy campaign?
...
### 1. What attack technique abuses the use of federated domains in Azure Active Directory (Entra ID), which was one of the attack techniques used during the SolarWinds hack?
...
### 1. What is the name of threat actors who typically has authorized access to an organization and uses it to harm that organization?
...
### 1. On October 13, 2023, which ransomware group advertised a hospital network on its darkweb leak site?
...
### 1. Look up MITRE ATT&CK technique T1040. What is the ID of the mitigation that suggests you should ensure traffic is encrypted properly?
...
[Resources > ATT&CKcon > ATT&CKcon 4.0](https://attack.mitre.org/resources/attackcon/october-2023/)
...
[Cyber Security Brief 23-11 - October 2023](https://cert.europa.eu/publications/threat-intelligence/cb23-11/#:~:text=The%20campaign%2C%20which%20was%20run,to%20be%20the%20exclusive%20user.)
...
### 1. On October 16, Cisco announced which CVE with a CVSS score of 10.0?
...
### 1. A well known espionage group, typically seen supporting Russia and Belarus, was seen targeting European governments using which webmail server zero-day?
...
### 1. On October 10, Citrix released which CVE with a CVSS score of 9.4?
...
### 1. In an old DFIR report published in September 2023, they identified an adversary moving from ScreenConnect usage to Hive Ransomware deployment in how many hours?
...
### 1. On October 25, 2023, Microsoft released a blog on which prolific threat actor known for extortion and social engineering tactics?
...
### 1. Who hosts the Microsoft Threat Intelligence Podcast?
...
To celebrate our 100th DAILY7 question, we will be giving away some prizes.
To answer this question, you will need to obtain words from different modules:
- From the **Sunlands** module, find the **role of mishepherd**. Copy the **FIRST word of their role**.
- From the **HopsNStuff** module, find the **email susanwirt sent to fredericksmith**. Copy the **FIFTH word of the subject line**.
- From the **SpookySweets** module, look for **wikipedia URLs that Patrick Slough visited**. Copy the **earliest topic** they looked at on Wikipedia (the part after the last slash / on the right).
